document.addEventListener("DOMContentLoaded", function() {
    ...
    function createTaskElement(id, title, content, status, createdAt, assigned_to,username) {
        ...
        if (status === 'todo') {
            ...
            addMoveToAssignButton(task); // Add this line
    ...
    // function createTaskElement(id, title, content, status, createdAt,assigned_to) {
    //     var task = document.createElement("div");
    //     task.classList.add("task");
    //     task.setAttribute("data-id", id);
    //     task.innerHTML = `<h3>${title}</h3><p>${content}</p><p><small>Created At: ${createdAt}</small></p><p><small>Assigned To: ${assigned_to}</small></p>`;
